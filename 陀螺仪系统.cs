...
namespace IngameScript
{
    ...
    class 陀螺仪瞄准系统
    {
        ...
        public void 初始化(导弹识别器 方块组, IMyControllerCompat 参考驾驶舱)
        {
            方块组.GetBlocksOfType(陀螺仪列表);
            this.参考驾驶舱 = 参考驾驶舱;
            初始化PID控制器();
        ...
        private void 瞄准(Vector3D 方向)
        {
            if (!已初始化) return;
            if (内部时钟 == 0)
            {
                陀螺仪列表.ForEach(g => { g.Enabled = true; g.GyroOverride = true; }); // 确保陀螺仪开启
            }
            内部时钟++;
            // 计算陀螺仪目标角度
